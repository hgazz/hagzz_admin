...
<div class="row">
    <div class="row">
        @foreach (\App\Services\TranslatableService::getTranslatableInputs(App\Models\Sport::class) as $name => $data)
            <div class="col-md-6 mb-3">
                ...
                <input type="text" id="{{$name}}" name="{{$name}}" maxlength="50" class="form-control"
                       value="@if ($name == 'name_en') {{old($name, $area?->getTranslation('name','en')  ?? '')}} @else {{old($name, $area?->getTranslation('name','ar')  ?? '')}} @endif"
        ...
        @endforeach
            <div class="col-md-6 mb-3">
                <label for="role">{{trans('admin.academies.level')}}</label>
        ...
        <div class="col-md-6 mb-3">
            <label for="icon">{{trans('admin.academies.icon')}}</label>
