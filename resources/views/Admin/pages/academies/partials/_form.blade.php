...
<div class="step subscription-step" data-step-panel="5">
    ...
    <div class="mb-3"><label>{{ trans('admin.saas.offer_name') }}</label><input type="text" name="offer_name" value="{{ old('offer_name',$currentSubscription->offer_name ?? '') }}" placeholder="{{ trans('admin.saas.offer_name_hint') }}"></div>
    <div class="mb-3"><label>{{ trans('admin.saas.free_months') }}</label><input id="free_months" type="number" min="0" max="60" name="free_months" value="{{ old('free_months',$currentSubscription->free_months ?? 0) }}"></div>
    <div class="mb-3"><label>{{ trans('admin.saas.discount_type') }}</label><select name="discount_type" class="form-control basic"><option value="">{{ trans('admin.saas.no_discount') }}</option><option value="percentage" @selected(old('discount_type',$currentSubscription->discount_type ?? '')==='percentage')>{{ trans('admin.saas.percentage') }}</option><option value="fixed" @selected(old('discount_type',$currentSubscription->discount_type ?? '')==='fixed')>{{ trans('admin.saas.fixed_amount') }}</option></select></div>
    <div class="mb-3"><label>{{ trans('admin.saas.discount_value') }}</label><input type="number" min="0" step="0.01" name="discount_value" value="{{ old('discount_value',$currentSubscription->discount_value ?? 0) }}"></div>
    <div class="mb-3"><label>{{ trans('admin.saas.discount_starts_at') }}</label><input type="date" name="discount_starts_at" value="{{ old('discount_starts_at',$currentSubscription?->discount_starts_at?->format('Y-m-d')) }}"></div>
    <div class="mb-3"><label>{{ trans('admin.saas.discount_ends_at') }}</label><input type="date" name="discount_ends_at" value="{{ old('discount_ends_at',$currentSubscription?->discount_ends_at?->format('Y-m-d')) }}"></div>
    ...
    <div class="mb-3"><label>{{ trans('admin.saas.billing_starts_at') }}</label><input id="billing_starts_at" type="date" name="billing_starts_at" value="{{ old('billing_starts_at',$currentSubscription?->billing_starts_at?->format('Y-m-d') ?? ($currentSubscription?->trial_ends_at?->copy()->addDay()->format('Y-m-d'))) }}"></div>
    <div class="mb-3"><label>{{ trans('admin.saas.grace_days') }}</label><input type="number" min="0" max="90" name="grace_days" value="{{ old('grace_days',$currentSubscription->grace_days ?? 7) }}"></div>
    <div class="mb-3"><label>{{ trans('admin.saas.subscription_status') }}</label><select name="subscription_status" class="form-control basic"><option value="active" @selected(old('subscription_status',$currentSubscription->status ?? 'active')==='active')>{{ trans('admin.saas.active') }}</option><option value="suspended" @selected(old('subscription_status',$currentSubscription->status ?? '')==='suspended')>{{ trans('admin.subscription_revenue.statuses.suspended') }}</option><option value="cancelled" @selected(old('subscription_status',$currentSubscription->status ?? '')==='cancelled')>{{ trans('admin.subscription_revenue.statuses.cancelled') }}</option></select></div>
    <div class="mb-3"><label>{{ trans('admin.saas.billing_notes') }}</label><textarea name="billing_notes" rows="2">{{ old('billing_notes',$currentSubscription->billing_notes ?? '') }}</textarea></div>
