...
$language = $name == 'question_en' ? 'en' : 'ar';
                           $defaultValue = isset($faq) ? $faq->getTranslation('question', $language) : ''; @endphp
                value="{{ old($name, $defaultValue) }}" placeholder="Enter {{ $name }}"
    ...
    @endforeach
    <div class="col-6">
        <label for="answer_en">{{ trans('admin.faq.answer_en') }}<code>*</code></label>
    ...
    <div class="col-6">
        <label for="answer_ar">{{ trans('admin.faq.answer_ar') }}<code>*</code></label>
