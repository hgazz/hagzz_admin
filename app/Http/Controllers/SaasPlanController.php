...
class SaasPlanController extends Controller
{
    ...
    private function validateData(Request $request, ?SaasPlan $plan = null): array
    {
        return $request->validate([
            ...
            'prices.*.currency_code' => ['required_if:prices.*.enabled,1','nullable','string','size:3'],
            'prices.*.monthly_price' => ['required_if:prices.*.enabled,1','nullable','numeric','min:0'],
            'prices.*.annual_price' => ['required_if:prices.*.enabled,1','nullable','numeric','min:0'],
    ...
    {
        $firstPrice = collect($data['prices'])->firstWhere('enabled', '1') ?? collect($data['prices'])->first();
